...
void real(Complejo c1, Complejo c2)
{
	printf("La parte real de los numeros complejos son: %0.2f y %0.2f", c1.real, c2.real);
}
void imag(Complejo c1, Complejo c2)
{
	printf("La parte imaginaria de los numeros complejos son: %0.2fi y %0.2fi", c1.imag, c2.imag);
}
...
void magAn(Complejo c)
{
	int res=0;
	float mag=0.0, ang=0.0;
	printf("Pasar a polar? si=1 / no=0: ");
	scanf("%d",&res);
	if(res==1)
	{
		convertirPolar(&mag,&ang,c.real,c.imag);
		c.magnitud=mag;
		c.angulo=ang;
		printf("magnitud= %0.2f, angulo= %0.2f\n", c.magnitud, c.angulo);
	}
	else
		return;
}
