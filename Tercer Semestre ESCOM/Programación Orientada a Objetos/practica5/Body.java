...
public class Body implements Movible {
    ...
    //LaserBeam lb;
    //RayoLaser rl;
    TransformGroup tg2armDer;
    TransformGroup tg2armIzq;
    TransformGroup tgmanoder;
    TransformGroup tgmanoizq;
    TransformGroup tglegabader;
    TransformGroup tg2legabaDer;
    TransformGroup tg2legabaIzq;
    TransformGroup tg2PUDer;
    TransformGroup tg2PUIzq;
    TransformGroup tgtoeder;
    TransformGroup tgtoeizq;
    BranchGroup bgBala;
    BranchGroup bgBala1;
    public void disparo(){
  ...
  // Updates the view point by the translation change of the sprite
  {
    ...
    try{
     ...
     public void giraBD(float angulo){
    try{
      if(angulo < Math.PI ){
          angulo+=0.2f;
          Transform3D t3d2=new Transform3D();
          t3d2.rotX(-angulo);
          tg2armDer.setTransform(t3d2);
      }
    }catch(Exception e){
    }
  }
  public void giraPUD(float angulo){
    try{
      if(angulo < Math.PI ){
          angulo+=0.2f;
          Transform3D t3d2=new Transform3D();
          t3d2.rotX(-angulo);
          tg2PUDer.setTransform(t3d2);
      }
    }catch(Exception e){
    }
  }
  public void giraPDD(float angulo){
    try{
      if(angulo < Math.PI ){
          angulo+=0.2f;
          Transform3D t3d2=new Transform3D();
          t3d2.rotX(-angulo);
          tg2legabaDer.setTransform(t3d2);
      }
    }catch(Exception e){
    }
  }
  ...
  public void giraBI(float angulo){
    try{
      if(angulo < Math.PI ){
          angulo+=0.2f;
          Transform3D t3d2=new Transform3D();
          t3d2.rotX(-angulo);
          tg2armIzq.setTransform(t3d2);
      }
    }catch(Exception e){
    }
  }
  public void giraPUI(float angulo){
    try{
      if(angulo < Math.PI ){
          angulo+=0.2f;
          Transform3D t3d2=new Transform3D();
          t3d2.rotX(-angulo);
          tg2PUIzq.setTransform(t3d2);
      }
    }catch(Exception e){
    }
  }
  public void giraPDI(float angulo){
    try{
      if(angulo < Math.PI ){
          angulo+=0.2f;
          Transform3D t3d2=new Transform3D();
          t3d2.rotX(-angulo);
          tg2legabaIzq.setTransform(t3d2);
      }
    }catch(Exception e){
    }
  }
    ...
    public Body(float x, float y, float z,String nombre_mostrar,boolean movimiento,Frame frame,String img/*, TransformGroup vTG*/){
	...
	tran= new TransformGroup();
        ...
        + Primitive.GENERATE_TEXTURE_COORDS;
         ...
         Parte perso=new Parte("cabezaTina",0,0,0);
         transformacion1.addChild(perso.mybody());
         tran.addChild(transformacion1);
         //Cuello
    ...
    Parte tronc=new Parte("bodyT2",0,0,0);
    tg1Tron.addChild(tronc.mybody());
    tran.addChild(tg3Tron);
    tg3Tron.addChild(tg2Tron);
    tg2Tron.addChild(tg1Tron);
    //Hombro Derecho
    TransformGroup tg1HD=new TransformGroup();
    tg2HD=new TransformGroup();
    Parte armarrider=new Parte("arm-arri-der",0,0,0);
    tg1Tron.addChild(articula(armarrider.mybody(), tg1HD, tg2HD, 
         new Vector3f(-3.0506113f, 4.137867f, -1.1013789f)));
    //Brazo Derecho
    TransformGroup tg1armDer=new TransformGroup();
    tg2armDer=new TransformGroup();
    Parte armabader=new Parte("arm-aba-der",0,0,0);
    tg1HD.addChild(articula(armabader.mybody(), tg1armDer, tg2armDer, 
         new Vector3f(0.0f, -4.101568f, 0.0f)));    
    //mano derecha
    Parte manoder=new Parte("mano-der",0,0,0);
    tgmanoder=Posi.translate(manoder.mybody(), new Vector3f(0.0f, -2.894051f, 0.0f));
    tg1armDer.addChild(tgmanoder);
    //Hombro Izquiedo
    TransformGroup tg1HI=new TransformGroup();
    tg2HI=new TransformGroup();
    Parte armarriizq=new Parte("arm-arri-izq",0,0,0);
    tg1Tron.addChild(articula(armarriizq.mybody(), tg1HI, tg2HI, 
         new Vector3f(3.0506113f, 4.137867f, -1.1013789f)));
    //Brazo Izquierdo
    TransformGroup tg1armIzq=new TransformGroup();
    tg2armIzq=new TransformGroup();
    Parte armabaizq=new Parte("arm-aba-izq",0,0,0);
    tg1HI.addChild(articula(armabaizq.mybody(), tg1armIzq, tg2armIzq, 
         new Vector3f(0.0f, -4.101568f, 0.0f)));
//mano izq
    Parte manoizq=new Parte("mano-izq",0,0,0);
    tgmanoizq=Posi.translate(manoizq.mybody(), new Vector3f(0.0f, -2.894051f, 0.0f));
    tg1armIzq.addChild(tgmanoizq);
//==============================================
         //Pierna Derecha
    TransformGroup tg1PUDer=new TransformGroup();
    tg2PUDer=new TransformGroup();
    Parte legarrider=new Parte("leg-arri-der",0,0,0);
    tg1Tron.addChild(articula(legarrider.mybody(), tg1PUDer, tg2PUDer, 
         new Vector3f(0.05f,-0.15f,0f)));
//===============================================
    TransformGroup tg1legabaDer=new TransformGroup();
    tg2legabaDer=new TransformGroup();
    Parte legabader=new Parte("leg-aba-der", 0.0f , 0.0f , 0.0f );
    tg1PUDer.addChild(articula(legabader.mybody(), tg1legabaDer, tg2legabaDer, 
         new Vector3f(-1.5f, -7.0f, 0.0f )));
//================================================

    /*Parte toeizq=new Parte("toe-izq",0,0,0);
    tgtoeizq=Posi.translate(toeizq.mybody(), 
    new Vector3f(0.0f, -2.03902229f, 2.5246391f));
    tg1legabaIzq.addChild(tgtoeizq);*/

    Parte toeder=new Parte("toe-der",0,0,0);
    tgtoeder=Posi.translate(toeder.mybody(), 
    new Vector3f(0.0f, -7.5f, 0f));
    tg1legabaDer.addChild(tgtoeder);
//======================================================
         //Pierna Izquierda
    TransformGroup tg1PUIzq=new TransformGroup();
    tg2PUIzq=new TransformGroup();
    Parte legarriizq=new Parte("leg-arri-izq",0,0,0);
    tg1Tron.addChild(articula(legarriizq.mybody(), tg1PUIzq, tg2PUIzq, 
         new Vector3f(0.05f,-0.15f,0f)));
//==================================================================
    TransformGroup tg1legabaIzq=new TransformGroup();
    tg2legabaIzq=new TransformGroup();
    Parte legabaizq=new Parte("leg-aba-izq", 0.0f , 0.0f , 0.0f );
    tg1PUIzq.addChild(articula(legabaizq.mybody(), tg1legabaIzq, tg2legabaIzq, 
         new Vector3f(1.5f, -7.0f, 0.0f )));
    ...
    TransformGroup articula(
    BranchGroup hijo, TransformGroup bro , TransformGroup rotar, Vector3f pos){
    Transform3D t3d3=new Transform3D();
    t3d3.set(1,pos);
    TransformGroup tg3=new TransformGroup(t3d3);
    Transform3D t3d2=new Transform3D();
    t3d2.rotX(Math.PI*2);
    rotar.setTransform(t3d2);
    rotar.setCapability(TransformGroup.ALLOW_TRANSFORM_WRITE);
    Transform3D t3d1=new Transform3D();
    t3d1.set(1,new Vector3d(0.0f, 0.0f, 0.0f));
    bro.setTransform(t3d1);
    bro.addChild(hijo);
    tg3.addChild(rotar);
    rotar.addChild(bro);
    return tg3;
